...
describe("native intrinsic forwarding", () => {
  it("forwards img, svg, and canvas contracts", () => {
    ...
    expect(batch).toContainEqual(["setCustomPropValue", 2, "src", "fixture.png"])
    expect(batch).toContainEqual(["setCustomPropValue", 2, "objectFit", "contain"])
    expect(batch).toContainEqual(["setCustomPropValue", 2, "alt", "fixture"])
    expect(batch).toContainEqual(["createElement", 3, "svg"])
    expect(batch).toContainEqual(["setCustomPropValue", 3, "src", "fixture.svg"])
  ...
  it("forwards input and textarea editor props and events", () => {
    ...
    expect(batch).toContainEqual(["setCustomPropValue", 2, "value", "hello"])
    expect(batch).toContainEqual(["setCustomPropValue", 2, "placeholder", "Type"])
    expect(batch).toContainEqual(["setCustomPropValue", 2, "readOnly", true])
    expect(batch).toContainEqual(["setCustomPropValue", 2, "theme", theme])
    ...
    expect(batch).toContainEqual(["setCustomPropValue", 3, "value", "multiline"])
    expect(batch).toContainEqual(["setCustomPropValue", 3, "minRows", 2])
    expect(batch).toContainEqual(["setCustomPropValue", 3, "maxRows", 6])
  ...
  it("forwards the anchored positioning contract", () => {
    ...
    expect(batch).toContainEqual(["setCustomPropValue", 1, "position", { x: 20, y: 30 }])
    expect(batch).toContainEqual(["setCustomPropValue", 1, "side", "bottom"])
    expect(batch).toContainEqual(["setCustomPropValue", 1, "align", "center"])
    expect(batch).toContainEqual(["setCustomPropValue", 1, "gap", 8])
    expect(batch).toContainEqual(["setCustomPropValue", 1, "anchor", "bottomCenter"])
    expect(batch).toContainEqual(["setCustomPropValue", 1, "offset", { x: 1, y: 2 }])
    expect(batch).toContainEqual(["setCustomPropValue", 1, "fit", "switch"])
    expect(batch).toContainEqual(["setCustomPropValue", 1, "snapMargin", 10])
    expect(batch).toContainEqual(["setCustomPropValue", 1, "deferred", true])
    expect(batch).toContainEqual(["setCustomPropValue", 1, "priority", 1])
    expect(batch).toContainEqual(["setCustomPropValue", 1, "occlude", true])
  ...
  it("forwards code, diff, and markdown native text contracts", () => {
    ...
    expect(batch).toContainEqual(["setCustomPropValue", 2, "code", "const solid = 2"])
    expect(batch).toContainEqual(["setCustomPropValue", 2, "language", "ts"])
    expect(batch).toContainEqual(["setCustomPropValue", 2, "path", "fixture.ts"])
    expect(batch).toContainEqual(["setCustomPropValue", 2, "showLineNumbers", true])
    expect(batch).toContainEqual(["setCustomPropValue", 2, "showHeader", true])
    expect(batch).toContainEqual(["setCustomPropValue", 2, "theme", theme])

    expect(batch).toContainEqual(["createElement", 3, "diff"])
    expect(batch).toContainEqual(["setCustomPropValue", 3, "wordDiff", true])
    expect(batch).toContainEqual(["setCustomPropValue", 3, "collapsedPaths", ["old.ts"]])
    expect(batch).toContainEqual(["setCustomPropValue", 3, "scroll", true])
    expect(batch).toContainEqual(["setCustomPropValue", 3, "maxLines", 100])
    ...
    expect(batch).toContainEqual(["setCustomPropValue", 4, "source", "# GPUIX Solid"])
    expect(batch).toContainEqual(["setCustomPropValue", 4, "theme", theme])
  ...
  it("forwards virtual-list props and clears removed values", () => {
    ...
    expect(batch).toContainEqual(["setCustomPropValue", 1, "alignment", "bottom"])
    expect(batch).toContainEqual(["setCustomPropValue", 1, "followTail", true])
    expect(batch).toContainEqual(["setCustomPropValue", 1, "overdraw", 4])
    expect(batch).toContainEqual(["setCustomPropValue", 1, "estimatedItemHeight", 28])
    ...
    expect(renderer.batches.at(-1)).toEqual([
      ["setCustomPropValue", 1, "followTail", null],
    ])
  })
})
