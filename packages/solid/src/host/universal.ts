...
const DIV_SEMANTIC_TAGS = new Set([
  "button", "hr", "section", "main", "header", "footer", "nav", "aside", "article",
  "ul", "ol", "li", "form", "fieldset", "legend", "figure", "figcaption", "a",
])
...
const SVG_ATTRIBUTE_NAMES = new Map<string, string>([
  ...
  ["fillRule", "fill-rule"], ["clipRule", "clip-rule"], ["stopColor", "stop-color"],
  ["stopOpacity", "stop-opacity"], ["clipPath", "clip-path"],
])

const OMITTED_SVG_ATTRIBUTES = new Set([
  "children", "ref", "key", "style", "class", "className", "classList", "testId",
  "tabIndex", "autoFocus", "src", "source",
])
...
const runtime = createRenderer<HostNode | HostParent>({
  createElement(tagName, staticProps) {
    ...
    if (staticProps) {
      for (const [name, value] of Object.entries(staticProps)) {
        setNativeProperty(node, name, value, undefined)
      }
    }
    return node
  },
  ...
  setProperty(node, name, value, previous) {
    ...
    setNativeProperty(node, name, value, previous)
  },
...
function parseNativeClassList<T>(value: T): NativeClassList | undefined {
  ...
  for (const [candidate, enabled] of Object.entries(Object(value))) parsed[candidate] = Boolean(enabled)
  return parsed
}
...
function normalizeNativeInlineStyle(style: NativeInlineStyleInput | undefined): StyleDesc | undefined {
  ...
  const {
    ...
    "flex-direction": cssFlexDirection, "flex-wrap": cssFlexWrap,
    "align-items": cssAlignItems, "align-self": cssAlignSelf,
    "align-content": cssAlignContent, "justify-content": cssJustifyContent,
    "background-color": cssBackgroundColor, "font-family": cssFontFamily,
    "font-weight": cssFontWeight, "text-align": cssTextAlign,
    "white-space": cssWhiteSpace, "overflow-x": cssOverflowX,
    "overflow-y": cssOverflowY, "pointer-events": cssPointerEvents,
