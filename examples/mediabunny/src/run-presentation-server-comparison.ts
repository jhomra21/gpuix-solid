...
const workDirectory = join(projectDirectory, ".presentation-server-benchmark")
...
try {
  ...
  const native = await runJson("src/run-presentation-server-native.ts", [fixturePath])
  const comparison = formatPresentationComparison(browser, native)

  await Bun.write(
    join(reportsDirectory, "presentation-server-browser.json"),
    JSON.stringify(browser, null, 2) + "\n",
  )
  await Bun.write(
    join(reportsDirectory, "presentation-server-gpuix.json"),
    JSON.stringify(native, null, 2) + "\n",
  )
  await Bun.write(
    join(reportsDirectory, "presentation-server-comparison.md"),
    comparison + "\n",
  )

  console.log(comparison)
  console.log("")
  console.log("Reports written to examples/mediabunny/reports/presentation-server-*.{json,md}")
} finally {
  await rm(workDirectory, { recursive: true, force: true })
}
