...
For the decoder-worker comparison in a Codex session, use the in-app browser instead of Playwright:

```bash
bun run bench:presentation:napi-webcodecs:decode-scaling
```
...
`bench:presentation:napi-webcodecs:decode-scaling` isolates decoder scheduling after the NodeAV queue-depth matrix. It feeds the same AVC packets into `@napi-rs/webcodecs` with `prefer-hardware`, whose `VideoDecoder` uses a persistent worker thread instead of a separate PromiseWorker job for each send/receive call. It compares decode throughput, first-frame latency, and p95 delivery latency against browser WebCodecs at 720p, 1080p, and 4K. For local/Codex runs it does not launch Playwright: the CLI prints a localhost URL for Codex's in-app browser, the page runs the browser half, posts its JSON report back to the CLI, and the native worker measurement then continues. Playwright remains only in the older automated browser/CI lanes that need a headless browser without Codex. This worker path is still a lower-bound architecture probe because the current napi-WebCodecs implementation downloads hardware-decoded frames to CPU memory before exposing `VideoFrame`. The next native experiment should preserve the VideoToolbox frame and export its IOSurface only if this worker lane shows that scheduling is worth carrying forward.
