...
async function main(): Promise<void> {
  ...
  try {
    ...
    await app.getByTestId("toolbar-menu-settings").click()
    assert.equal(await app.getByTestId("toolbar-menu-content").count(), 0)
    await requireTestId(app, "toolbar-settings-dialog")
    assert.equal(await app.getByTestId("toolbar-theme-mode-value").textContent(), "dark")
    await app.getByTestId("toolbar-theme-light").click()
    assert.equal(await app.getByTestId("toolbar-theme-mode-value").textContent(), "light")
    await app.getByTestId("toolbar-locale-italiano").click()
    assert.equal(await app.getByTestId("toolbar-locale-value").textContent(), "Italiano")
    await app.getByTestId("toolbar-settings-close").click()
    assert.equal(await app.getByTestId("toolbar-settings-dialog").count(), 0)

    await app.getByTestId("toolbar-settings").click()
    await app.getByTestId("toolbar-menu-changelog").click()
    await requireTestId(app, "toolbar-changelog-dialog")
    assert.equal(await app.getByText("v1.9.0").count(), 1)
    await app.getByTestId("toolbar-changelog-close").click()

    await app.getByTestId("dashboard-link").click()
    assert.equal(await app.getByTestId("toolbar-navigation-status").textContent(), "Dashboard navigation selected")
    await app.getByTestId("toolbar-settings").click()
    await app.getByTestId("toolbar-menu-github").click()
    assert.equal(await app.getByTestId("toolbar-navigation-status").textContent(), "GitHub external link selected")

    await app.getByTestId("user-badge").click()
    await requireTestId(app, "user-badge-menu")
    await app.getByTestId("user-badge-logout").click()
    assert.equal(await app.getByTestId("user-badge").count(), 0)
    assert.equal(await app.getByTestId("dashboard-link").count(), 0)
    await requireTestId(app, "github-sign-in")
    assert.equal(await app.getByTestId("toolbar-navigation-status").textContent(), "Signed out locally")
    await app.getByTestId("github-sign-in").click()
    await requireTestId(app, "user-badge")
    await requireTestId(app, "dashboard-link")
    assert.equal(await app.getByTestId("github-sign-in").count(), 0)
    ...
    await app.getByText("Daily snippet").click()
