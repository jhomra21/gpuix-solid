...
The current parity pass is based on Kobalte docs source revision `3d3266348816b492b027538168988703dc1604c0` from July 18, 2026. Preserve both the component tokens and the example composition from those demos when changing this fixture:

- Button and button-like triggers: intrinsic width, 40px tall, 6px radius, 16px text, Kobalte blue (`hsl(200 98% 39%)` light / `hsl(201 96% 32%)` dark).
...
- Dialog: full-window 20%-black overlay, centered 500px content surface, 6px radius, 16px padding, 20px/500 title and 16px description.
...
8. The context target visibly uses a dashed 300px native frame and the context menu opens only from a real right-click.
...
10. Dialog uses a window-level overlay rather than dimming only its fixture cell, is centered in the native window, and closes from its close button or Escape.

## Current compatibility boundary

This is a Kobalte-shaped native compatibility layer, not a DOM implementation of Kobalte. Dropdown, context, tooltip and menubar portal content is represented by native floating/anchored layers rather than browser portals. Dialog uses a dedicated full-window native anchored layer so its overlay and centered positioner reproduce Kobalte's fixed-window modal behavior.
