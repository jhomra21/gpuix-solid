...
if (!hasNativeTestRenderer) {
  ...
  const openDropdownTrigger = r.boundsTestId("dropdown-trigger")
  const dropdownContent = r.boundsTestId("dropdown-content")
  requireCondition(closeEnough(openDropdownTrigger.x, dropdownContent.x), "DropdownMenu should stay left-aligned with its trigger")
  requireCondition(dropdownContent.y >= openDropdownTrigger.y + openDropdownTrigger.height, "DropdownMenu should open below its trigger")
  ...
  const viewport = r.boundsTestId("kobalte-showcase")
  const overlay = r.boundsTestId("dialog-overlay")
  const dialog = r.boundsTestId("dialog-content")
  requireCondition(closeEnough(viewport.x, overlay.x) && closeEnough(viewport.y, overlay.y), "Dialog overlay should start at the viewport origin")
  requireCondition(closeEnough(viewport.width, overlay.width) && closeEnough(viewport.height, overlay.height), `Dialog overlay should span the viewport; got ${overlay.width}x${overlay.height} for ${viewport.width}x${viewport.height}`)
  requireCondition(closeEnough(centerX(overlay), centerX(dialog), 3), "Dialog should be horizontally centered in the viewport")
  requireCondition(closeEnough(centerY(overlay), centerY(dialog), 3), "Dialog should be vertically centered in the viewport")
