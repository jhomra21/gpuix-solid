...
function ShowcaseBody(): JSX.Element {
  ...
  const buttonStyle = () => ({
    ...
    color: colorMode() === "dark" ? "rgba(255, 255, 255, 0.9)" : "#ffffff",
  ...
  const fieldStyle = (invalid = false) => ({
    ...
    paddingRight: 12,
    borderWidth: 1,
    borderColor: invalid ? palette().red : (colorMode() === "dark" ? "#52525b" : palette().border),
    borderRadius: 6,
    backgroundColor: palette().surface,
    color: invalid ? palette().red : palette().text,
    fontSize: 16,
    hover: { borderColor: colorMode() === "dark" ? "#71717a" : "#a1a1aa" },
  })
  const popupStyle = () => ({
    minWidth: 220,
    padding: 8,
    ...
    color: palette().text,
    borderWidth: 1,
    borderColor: colorMode() === "dark" ? "#3f3f46" : palette().border,
    borderRadius: 6,
  ...
  const separatorStyle = () => ({
    ...
    marginLeft: 6,
    marginRight: 6,
    backgroundColor: colorMode() === "dark" ? "#52525b" : palette().border,
  })
  ...
  return (
    <div testId="kobalte-showcase" style={{
      width: "100%",
      height: "100%",
      padding: 28,
      gap: 20,
      ...
      <Section title="Menubar">
        <Menubar.Root onValueChange={(value) => { if (value != null) setLastAction(`Menubar: ${value}`) }}>
          <Menubar.Menu value="file">
            <Menubar.Trigger testId="menubar-file" style={menubarTriggerStyle("first")}>
              <text style={{ fontSize: 16, color: palette().text }}>Git</text>
            </Menubar.Trigger>
            <Menubar.Portal>
              <Menubar.Content testId="menubar-file-content" style={popupStyle()}>
                <Menubar.Item testId="menubar-new" style={menuItemStyle()} onSelect={() => setLastAction("Commit")}>
                  <text style={{ fontSize: 16, color: palette().text }}>Commit</text>
                </Menubar.Item>
                <Menubar.Item style={menuItemStyle()} onSelect={() => setLastAction("Push")}>
                  <text style={{ fontSize: 16, color: palette().text }}>Push</text>
                </Menubar.Item>
                <Menubar.Separator style={separatorStyle()} />
                <Menubar.Sub>
                  <Menubar.SubTrigger testId="menubar-export" style={menuItemStyle()}>
                    <text style={{ fontSize: 16, color: palette().text }}>GitHub  ›</text>
                  </Menubar.SubTrigger>
                  <Menubar.SubContent testId="menubar-export-content" style={popupStyle()}>
                    <Menubar.Item style={menuItemStyle()} onSelect={() => setLastAction("Create Pull Request")}>
                      <text style={{ fontSize: 16, color: palette().text }}>Create Pull Request…</text>
                    </Menubar.Item>
                  </Menubar.SubContent>
                </Menubar.Sub>
              </Menubar.Content>
          ...
          <Menubar.Menu value="edit">
            <Menubar.Trigger testId="menubar-edit" style={menubarTriggerStyle("last")}>
              <text style={{ fontSize: 16, color: palette().text }}>Edit</text>
            </Menubar.Trigger>
      ...
      <div style={{ display: "flex", flexDirection: "row", gap: 16, alignItems: "stretch" }}>
        <div style={{ flexGrow: 1, gap: 16 }}>
          <Section title="Button + Tooltip + Separator">
            <div style={{ display: "flex", flexDirection: "row", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
              <Button.Root testId="button-action" onPress={() => setLastAction("Button pressed")} style={buttonStyle()}>
                <text style={buttonTextStyle()}>Click me</text>
              </Button.Root>
              <Button.Root testId="button-disabled" disabled style={{ ...buttonStyle(), opacity: 0.5 }}>
                <text style={buttonTextStyle()}>Disabled</text>
              </Button.Root>
              <Separator.Root orientation="vertical" style={{ width: 1, height: 40, backgroundColor: colorMode() === "dark" ? "#52525b" : palette().border }} />
              <Tooltip.Root openDelay={0} closeDelay={0} placement="top">
                <Tooltip.Trigger testId="tooltip-trigger" style={buttonStyle()}>
                  <text style={buttonTextStyle()}>Trigger</text>
                </Tooltip.Trigger>
                <Tooltip.Portal>
                  <Tooltip.Content testId="tooltip-content" style={{
                    maxWidth: 380,
                    padding: 8,
                    borderWidth: 1,
                    borderColor: colorMode() === "dark" ? "#3f3f46" : "#d4d4d8",
                    borderRadius: 6,
                    backgroundColor: palette().tooltip,
                    color: palette().tooltipText,
                    boxShadow: colorMode() === "dark"
                      ? undefined
                      : { offsetX: 0, offsetY: 10, blurRadius: 15, spreadRadius: -3, color: "rgba(0, 0, 0, 0.1)" },
                  }}>
          ...
          <Section title="Text Field">
            <div style={{ display: "flex", flexDirection: "row", gap: 28, alignItems: "flex-start", flexWrap: "wrap" }}>
              <TextField.Root value={name()} onValueChange={setName}>
                <TextField.Label style={{ color: colorMode() === "dark" ? "#d4d4d8" : "#18181b", fontSize: 14, fontWeight: 500 }}>Favorite fruit</TextField.Label>
                <TextField.Input testId="text-field-input" placeholder="Apple" style={fieldStyle()} />
                <TextField.Description style={{ color: colorMode() === "dark" ? "#a1a1aa" : "#3f3f46", fontSize: 12 }}>Enter your favorite fruit.</TextField.Description>
              </TextField.Root>
              <TextField.Root validationState="invalid" defaultValue="Bad route">
                <TextField.Label style={{ color: colorMode() === "dark" ? "#d4d4d8" : "#18181b", fontSize: 14, fontWeight: 500 }}>Invalid field</TextField.Label>
          ...
          <Section title="Image + Separator">
            <div style={{ display: "flex", flexDirection: "row", gap: 8, alignItems: "center" }}>
              <Image.Root testId="avatar-jm" style={{ width: 56, height: 56, borderRadius: 28, overflow: "hidden", backgroundColor: palette().imageFallback }}>
                <Image.Fallback testId="avatar-jm-content" style={{ width: "100%", height: "100%", alignItems: "center", justifyContent: "center", backgroundColor: palette().imageFallback }}>
                  <text style={{ fontSize: 16, lineHeight: 20, fontWeight: 500, color: palette().imageFallbackText }}>KB</text>
                </Image.Fallback>
              </Image.Root>
              <Image.Root testId="avatar-fallback" style={{ width: 56, height: 56, borderRadius: 28, overflow: "hidden", backgroundColor: palette().imageFallback }}>
                <Image.Fallback testId="avatar-fallback-content" style={{ width: "100%", height: "100%", alignItems: "center", justifyContent: "center", backgroundColor: palette().imageFallback }}>
                  <text style={{ fontSize: 16, lineHeight: 20, fontWeight: 500, color: palette().imageFallbackText }}>J</text>
                </Image.Fallback>
              </Image.Root>
              <Separator.Root orientation="vertical" style={{ width: 1, height: 56, marginLeft: 8, marginRight: 8, backgroundColor: colorMode() === "dark" ? "#52525b" : palette().border }} />
        ...
        <div style={{ width: 430, minWidth: 430, gap: 16 }}>
          <Section title="Dropdown Menu">
            <DropdownMenu.Root>
              <DropdownMenu.Trigger testId="dropdown-trigger" style={buttonStyle()}>
                <text style={buttonTextStyle()}>Git Settings ⌄</text>
              </DropdownMenu.Trigger>
              <DropdownMenu.Portal>
                <DropdownMenu.Content testId="dropdown-content" style={popupStyle()}>
                  <DropdownMenu.Item testId="dropdown-item" style={menuItemStyle()} onSelect={() => setLastAction("Commit")}>
                    <text style={{ fontSize: 16, color: palette().text }}>Commit</text>
                  </DropdownMenu.Item>
                  <DropdownMenu.Item style={menuItemStyle()} onSelect={() => setLastAction("Push")}>
                    <text style={{ fontSize: 16, color: palette().text }}>Push</text>
                  </DropdownMenu.Item>
                  ...
                  <DropdownMenu.RadioGroup value={radio()} onChange={setRadio}>
                    <DropdownMenu.RadioItem testId="radio-beats" style={menuItemStyle()} value="main">
                      <DropdownMenu.ItemIndicator testId="radio-beats-indicator" style={indicatorStyle()}><text style={{ fontSize: 12, color: palette().text }}>●</text></DropdownMenu.ItemIndicator>
                      <text style={{ fontSize: 16, color: palette().text }}>main</text>
                    </DropdownMenu.RadioItem>
                    <DropdownMenu.RadioItem testId="radio-time" style={menuItemStyle()} value="develop">
                      <DropdownMenu.ItemIndicator testId="radio-time-indicator" style={indicatorStyle()}><text style={{ fontSize: 12, color: palette().text }}>●</text></DropdownMenu.ItemIndicator>
                      <text style={{ fontSize: 16, color: palette().text }}>develop</text>
                    </DropdownMenu.RadioItem>
                  </DropdownMenu.RadioGroup>
                  <DropdownMenu.Sub>
                    <DropdownMenu.SubTrigger testId="dropdown-sub-trigger" style={menuItemStyle()}>
                      <text style={{ fontSize: 16, color: palette().text }}>GitHub  ›</text>
                    </DropdownMenu.SubTrigger>
                    <DropdownMenu.SubContent testId="dropdown-sub-content" style={popupStyle()}>
                      <DropdownMenu.Item style={menuItemStyle()} onSelect={() => setLastAction("Create Pull Request")}>
                        <text style={{ fontSize: 16, color: palette().text }}>Create Pull Request…</text>
                      </DropdownMenu.Item>
                      <DropdownMenu.Item style={menuItemStyle()} onSelect={() => setLastAction("View Pull Requests")}>
                        <text style={{ fontSize: 16, color: palette().text }}>View Pull Requests</text>
                      </DropdownMenu.Item>
          ...
          <Section title="Context Menu">
            <ContextMenu.Root>
              <ContextMenu.Trigger testId="context-trigger" style={{
                width: 300,
                height: 105,
                minHeight: 105,
                alignItems: "center",
                justifyContent: "center",
                borderWidth: 2,
                borderColor: colorMode() === "dark" ? "rgba(255, 255, 255, 0.5)" : "#71717a",
                borderRadius: 4,
                color: colorMode() === "dark" ? "rgba(255, 255, 255, 0.7)" : "#52525b",
                userSelect: "none",
              }}>
                <text style={{ fontSize: 15, color: colorMode() === "dark" ? "rgba(255, 255, 255, 0.7)" : "#52525b" }}>Right click here.</text>
              </ContextMenu.Trigger>
              <ContextMenu.Content testId="context-content" style={popupStyle()}>
                <ContextMenu.Item testId="context-duplicate" style={menuItemStyle()} onSelect={() => setLastAction("Commit")}>
                  <text style={{ fontSize: 16, color: palette().text }}>Commit</text>
                </ContextMenu.Item>
                <ContextMenu.Item style={menuItemStyle()} onSelect={() => setLastAction("Push")}>
                  <text style={{ fontSize: 16, color: palette().text }}>Push</text>
                </ContextMenu.Item>
                <ContextMenu.Separator style={separatorStyle()} />
                <ContextMenu.Item style={menuItemStyle()} onSelect={() => setLastAction("Update Project")}>
                  <text style={{ fontSize: 16, color: palette().subtle }}>Update Project</text>
                </ContextMenu.Item>
              </ContextMenu.Content>
            </ContextMenu.Root>
          </Section>
          ...
          <Section title="Dialog">
            <Dialog.Root>
              ...
              <Dialog.Portal>
                ...
                <Dialog.Content testId="dialog-content" style={{
                  width: 500,
                  padding: 16,
                  gap: 12,
                  backgroundColor: palette().surface,
                  ...
                  <div style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                    <Dialog.Title style={{ fontSize: 20, lineHeight: 26, fontWeight: 500, color: colorMode() === "dark" ? "rgba(255, 255, 255, 0.9)" : "#18181b" }}>About Kobalte</Dialog.Title>
                    <Dialog.CloseButton testId="dialog-close" style={{ width: 25, height: 25, minWidth: 25, minHeight: 25, borderRadius: 4, color: colorMode() === "dark" ? "rgba(255, 255, 255, 0.8)" : "#52525b", hover: { backgroundColor: colorMode() === "dark" ? "#3f3f46" : "#f4f4f5" } }}>
                      <text style={{ fontSize: 18, color: colorMode() === "dark" ? "rgba(255, 255, 255, 0.8)" : "#52525b" }}>×</text>
                    </Dialog.CloseButton>
                  </div>
                  <Dialog.Description style={{ fontSize: 16, lineHeight: 23, color: colorMode() === "dark" ? "rgba(255, 255, 255, 0.7)" : "#3f3f46" }}>
