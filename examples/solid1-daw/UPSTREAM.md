...
Semantic `<canvas>` creation is intercepted only by the DAW universal module. With published `@gpuix/native@0.9.0`, the facade records the static operations exercised by the pinned source and presents them through the existing SVG/image compatibility path. With the audited source-edge Canvas v5 capability, a mounted semantic canvas is instead adopted as GPUIX's native `canvas` custom element and the same copied source records directly into the versioned native draw list. The fallback remains available when the native capability is absent; it is not mistaken for native support.

The source-edge detector proves that waveform and EQ drawing select the native path, that no SVG/image fallback surface is created, that partial arcs use the native cubic-path lowering, and that opaque full-frame EQ repaints discard fully occluded retained commands. The stable detector continues to prove the SVG/image fallback independently.
