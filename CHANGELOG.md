...
- Expose GPUIX native Finder/OS file drops as `onFileDrop` in both Solid renderer hosts, with GPU-backed regression coverage and Solid 2 locator `dropFiles()` automation. Add internal semantic drag/drop through `dragData`, `dragPreview`, `onDragStart`, `onDragOver`, `onDrop`, and `onDragEnd`, using pointer-geometry drop targeting, a default pointer-following preview chip, automatic draggable text-selection suppression, and click suppression after a completed drag.
