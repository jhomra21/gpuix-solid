...
- Add an experimental native Canvas2D source-edge path with a versioned browser-shaped draw-list recorder in both Solid hosts, bounded opaque full-frame repaint retention, an isolated GPUIX patch, capability-based native adoption, and GPU-backed paint and pointer acceptance. Protocol v5 keeps affine transforms on text commands so Diffusion HUD labels can rotate and scale through GPUI, while retaining the rounded clipping added for its timeline. The Solid 1 DAW source-edge detector exercises native waveform and EQ draw lists while the default npm dependency remains `@gpuix/native@0.9.0`.
